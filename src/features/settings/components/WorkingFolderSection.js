...
export const WorkingFolderSection = memo(() => {
  ...
  const saveFolder = useCallback(
    async (path) => {
      ...
      setError('');
      try {
        await apiPost('/api/settings', { ...settings, working_directory: path });
        await refetch();
        setValue(path);
        setEditing(false);
        toast.success(
  ...
  return _jsxs('div', {
    ...
    children: [
      _jsxs('div', {
        ...
        children: [
          _jsx(FolderOpen, { size: 18, className: 'text-[var(--matrix-accent)]' }),
          _jsx('h3', {
      ...
      editing
            ...
            children: [
              ...
              error &&
                _jsxs('div', {
                  ...
                  children: [_jsx(AlertCircle, { size: 14 }), _jsx('span', { className: 'text-xs', children: error })],
                }),
              _jsxs('div', {
        ...
        : _jsxs('div', {
            ...
            children: [
              currentFolder
                ? _jsx('div', {
                    className: cn('text-sm font-mono px-3 py-2 rounded-lg bg-[var(--matrix-glass)]', theme.text),
                    children: currentFolder,
                  })
                : _jsx('p', {
                    className: cn('text-xs italic', theme.textMuted),
                    children: t('settings.workingFolder.notSet', 'Not set — agents will require absolute paths'),
                  }),
              _jsxs('div', {
                className: 'flex gap-2',
                children: [
                    ...
                    _jsxs(_Fragment, {
                      children: [
                        _jsx(Button, {
                          variant: 'ghost',
                          size: 'sm',
                          leftIcon: _jsx(Pencil, { size: 14 }),
                          onClick: () => setEditing(true),
                          disabled: browsing || saving,
                          children: t('settings.workingFolder.editManually', 'Edit'),
                        }),
                        _jsx(Button, {
                          variant: 'danger',
                          size: 'sm',
