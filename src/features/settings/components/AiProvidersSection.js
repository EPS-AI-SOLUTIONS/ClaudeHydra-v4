      ...
      children: [
        _jsxs('div', {
          ...
          children: [
            _jsxs('div', {
              ...
              children: [
                ...
                _jsxs('div', {
                  ...
                  children: [
                    _jsx('h4', {
                      className: cn('text-sm font-semibold font-mono truncate', theme.text),
            ...
            _jsx(Badge, {
              ...
              icon:
                status === 'connected'
                  ? _jsx(CheckCircle, { size: 10 })
                  : status === 'error'
                    ? _jsx(AlertTriangle, { size: 10 })
                    : status === 'expiring'
                      ? _jsx(Clock, { size: 10 })
                      : _jsx(WifiOff, { size: 10 }),
        ...
        _jsxs('div', {
          ...
          children: [
            ...
            _jsx('span', { className: cn('text-[10px] font-mono ml-auto', theme.textMuted), children: p.auth_type }),
          ],
        }),
        _jsxs('div', {
          className: 'flex items-center gap-1.5',
          children: [
            _jsx(Clock, { size: 10, className: theme.textMuted }),
            _jsxs('span', {
              className: cn('text-[10px] font-mono', theme.textMuted),
              children: [t('settings.providers.lastVerified', 'Verified'), ': ', formatVerified(p.last_verified)],
            }),
          ],
        }),
        p.last_error &&
          _jsxs('div', {
            ...
            children: [
              _jsx(AlertTriangle, { size: 12, className: 'shrink-0 mt-0.5' }),
              _jsx('span', { className: 'text-[10px] font-mono leading-tight line-clamp-2', children: p.last_error }),
            ],
          }),
        _jsx('div', {
...
const SummaryFooter = memo(({ providers }) => {
  ...
  return _jsxs('div', {
    className: cn('flex items-center justify-between flex-wrap gap-3 pt-4 mt-2', 'border-t border-white/10'),
    children: [
      _jsxs('div', {
        className: 'flex items-center gap-2',
        children: [
          _jsx(Zap, { size: 14, className: 'text-[var(--matrix-accent)]' }),
          _jsx('span', {
            className: cn('text-xs font-mono', theme.text),
            children: t('settings.providers.connectedCount', '{{count}}/{{total}} connected', {
      ...
      _jsx('div', {
        ...
        children: _jsxs('span', {
          ...
          children: [t('settings.providers.totalCost', 'Total'), ': $', (totalCostCents / 100).toFixed(0), '/mo'],
        }),
      }),
      _jsxs('div', {
        className: 'flex items-center gap-1.5',
        children: [
          _jsx('div', {
            className: cn('w-2 h-2 rounded-full', hasErrors ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'),
          }),
          _jsx('span', {
            className: cn('text-[10px] font-mono', theme.textMuted),
...
export default function AiProvidersSection() {
  ...
  // ── Handlers ──
  const handleConnect = useCallback((provider) => connectMutation.mutate(provider), [connectMutation]);
  const handleDisconnect = useCallback((provider) => disconnectMutation.mutate(provider), [disconnectMutation]);
  const handleTest = useCallback((provider) => testMutation.mutate(provider), [testMutation]);
  // ── Render ──
  return _jsxs('div', {
    className: 'space-y-5',
    children: [
      _jsxs('div', {
        ...
        children: [
          _jsxs('div', {
            className: 'flex items-center gap-2',
            children: [
              _jsx(Shield, { size: 18, className: 'text-[var(--matrix-accent)]' }),
              _jsx('h3', {
          ...
          _jsx(Button, {
            variant: 'ghost',
            size: 'sm',
            leftIcon: _jsx(RefreshCw, { size: 12, className: isLoading ? 'animate-spin' : '' }),
            onClick: () => refetch(),
            disabled: isLoading,
            className: 'text-xs',
      ...
      isLoading &&
        ...
        _jsxs('div', {
          ...
          children: [
            _jsx(Loader2, { size: 20, className: 'text-[var(--matrix-accent)] animate-spin' }),
            _jsx('span', {
        ...
        _jsxs('div', {
          ...
          children: [
            ...
            _jsx('span', {
              ...
              children: t('settings.providers.fetchError', 'Failed to load providers. Check backend connection.'),
            }),
          ],
        }),
      providers &&
      ...
      providers &&
        ...
        _jsxs('div', {
          ...
          children: [
            _jsx(WifiOff, { size: 24, className: theme.textMuted }),
            _jsx('span', {
              className: cn('text-sm font-mono', theme.textMuted),
              children: t('settings.providers.empty', 'No providers configured'),
            }),
          ],
        }),
      providers && providers.length > 0 && _jsx(SummaryFooter, { providers: providers }),
    ],
  });
}
