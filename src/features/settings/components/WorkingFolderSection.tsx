...
export const WorkingFolderSection = memo(() => {
  ...
  const handleSave = useCallback(async () => {
    ...
    setError('');
    try {
      await apiPost<Settings>('/api/settings', { ...settings, working_directory: value.trim() });
      await refetch();
      setEditing(false);
  ...
  const handleClear = useCallback(async () => {
    ...
    setError('');
    try {
      await apiPost<Settings>('/api/settings', { ...settings, working_directory: '' });
      await refetch();
      setValue('');
      setEditing(false);
