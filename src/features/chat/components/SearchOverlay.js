...
export function SearchOverlay({ messages, onMatchChange, onClose }) {
  ...
  return _jsxs(motion.div, {
    ...
    children: [
      ...
      query &&
        _jsx('span', {
          className: 'text-xs text-[var(--matrix-text-secondary)] font-mono whitespace-nowrap',
          children: matches.length > 0 ? `${currentMatchIdx + 1}/${matches.length}` : '0/0',
        }),
      _jsxs('div', {
        ...
        children: [
          _jsx('button', {
            ...
            children: _jsx(ChevronUp, { size: 14 }),
          }),
          _jsx('button', {
            type: 'button',
            onClick: goNext,
...
export function highlightText(text, query) {
  ...
  return parts.map((part, i) => {
    if (part.toLowerCase() === query.toLowerCase()) {
      return _jsx(
        'mark',
        { className: 'bg-yellow-400/40 text-inherit rounded px-0.5', children: part },
