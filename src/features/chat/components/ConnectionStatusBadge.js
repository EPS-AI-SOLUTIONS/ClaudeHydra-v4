...
export const ConnectionStatusBadge = memo(function ConnectionStatusBadge({
  ...
  if (connectionStatus === 'connected') {
    return _jsxs('div', {
      ...
      children: [
        _jsx('span', { className: `inline-block w-1.5 h-1.5 rounded-full ${config.dotClass}` }),
        _jsx(Wifi, { size: 12, className: config.color }),
      ],
    });
  }
  return _jsxs(motion.div, {
    ...
    children: [
      connectionStatus === 'reconnecting'
        ...
        : _jsx('span', { className: `inline-block w-1.5 h-1.5 rounded-full ${config.dotClass}` }),
      connectionStatus === 'reconnecting'
        ? _jsx(RefreshCw, { size: 12, className: 'animate-spin' })
