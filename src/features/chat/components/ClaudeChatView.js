 ...
 */
import { AgentActivityPanel, ArtifactPanel, EMPTY_ACTIVITY, usePromptHistory } from '@jaskier/chat-module';
...
import { FALLBACK_CLAUDE_MODELS, useClaudeModels } from '@/features/chat/hooks/useClaudeModels';
...
export function ClaudeChatView() {
  ...
  const activeArtifact = useViewStore(useShallow((s) => s.activeArtifact));
  ...
  const handleSearchMatchChange = useCallback((messageId) => {
  ...
  const [agentActivity, setAgentActivity] = useState(EMPTY_ACTIVITY);
  const [fallbackBanner, setFallbackBanner] = useState(null);
  const tokenBatchRef = useRef('');
  const flushTimerRef = useRef(null);
  const handleFallback = useCallback((info) => {
  ...
  const ws = useWebSocketChat({
    onStart: (msg) => {
      ...
      if (sid) {
        messageState.updateSessionMessages(sid, (prev) =>
          prev.map((m) => (m.status === 'pending' ? { ...m, status: 'confirmed' } : m)),
        );
      }
    },
    onToken: (content) => {
    ...
    onError: (message) => {
      ...
      if (sid) {
        messageState.updateSessionMessages(sid, (prev) => {
          ...
          return withoutStreaming.map((m) => (m.status === 'pending' ? { ...m, status: 'error' } : m));
        });
        messageState.setSessionLoading(sid, false);
      }
      toast.error(message);
  ...
  const wsHandleSend = useCallback(
    async (text, attachments) => {
      ...
      messageState.updateSessionMessages(sessionId, (prev) => {
        ...
        return prev.map((m) => (m.id === userMessageId ? { ...m, status: 'confirmed' } : m)).concat(assistantMessage);
      });
  ...
  // ----- Render -------------------------------------------------------------
  return _jsxs(CompletionFeedback, {
    ...
    children: [
      ...
      _jsx(FallbackBanner, { data: fallbackBanner, onDismiss: dismissFallbackBanner }),
      _jsxs('div', {
        className: 'flex-1 min-h-0 flex relative overflow-hidden gap-2',
        children: [
          _jsx(VirtualizedMessageArea, {
            messages: messages,
          ...
          _jsx(ArtifactPanel, { activeArtifact: activeArtifact, onClose: handleCloseArtifact }),
        ],
      }),
      _jsx(StreamingIndicator, { isStreaming: effectiveLoading }),
      _jsx(AnimatePresence, {
        children:
          (agentActivity.isActive || agentActivity.tools.length > 0) &&
          _jsx('div', { className: 'mt-2', children: _jsx(AgentActivityPanel, { activity: agentActivity }) }),
      }),
      _jsx('div', {
        className: 'mt-3',
