  ...
  return _jsxs(motion.div, {
    ...
    children: [
      attachment.type === 'image'
        ...
        : _jsx(FileText, { size: 16, className: 'text-blue-400 shrink-0' }),
      _jsx('span', {
        className: 'text-sm truncate max-w-[150px] text-[var(--matrix-text-primary)]',
...
// Component
// ---------------------------------------------------------------------------
export const AttachmentPreview = memo(function AttachmentPreview({ attachments, onRemove }) {
  if (attachments.length === 0) return null;
  return _jsx('div', {
    className: 'flex flex-wrap gap-2',
    children: attachments.map((att) => _jsx(AttachmentChip, { attachment: att, onRemove: onRemove }, att.id)),
  });
});
