...
const viewImportMap: Record<string, () => Promise<unknown>> = {
 ...
 */
function prefetchView(viewId: string): void {
...
export function usePredictivePrefetch() {
  ...
  useEffect(() => {
    function handleViewHint(e: Event) {
      const detail = (e as CustomEvent<{ views: string[] }>).detail;
      if (detail?.views) {
   ...
   */
  const prefetchOnHover = useCallback((viewId: ViewId) => {
    if (prefetchedChunks.has(viewId)) return;
 ...
 */
export function dispatchViewHint(views: string[]): void {
