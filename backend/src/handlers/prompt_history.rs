...
#[utoipa::path(post, path = "/api/prompt-history", tag = "chat",
    request_body = AddPromptRequest,
    responses((status = 201, description = "Prompt added")))]
pub async fn add_prompt_history(
    ...
    // Consecutive dedup — skip if identical to last entry
    ...
    // Cap at MAX_PROMPT_HISTORY entries
    sqlx::query(
...
#[utoipa::path(delete, path = "/api/prompt-history", tag = "chat",
