...
pub async fn audit_logs(
    ...
    let total: i64 = sqlx::query_scalar(
        concat!(
            "SELECT COUNT(*)::bigint FROM ch_audit_log",
    ...
    let rows = sqlx::query_as::<_, (i32, chrono::DateTime<chrono::Utc>, String, Option<Value>, Option<String>)>(
        concat!(
            "SELECT id, timestamp, action, details, ip_address FROM ch_audit_log",
            " WHERE ($1::text IS NULL OR action = $1)",
            " AND ($2::text IS NULL OR details::text ILIKE '%' || $2 || '%')",
...
pub async fn flyio_logs(
    ...
    // Read PAT from service_tokens DB or env var fallback
    let token = match crate::service_tokens::get_service_token(&state, "flyio").await {
        Some(t) => t,
        None => match std::env::var("FLY_API_TOKEN") {
            Ok(t) if !t.is_empty() => t,
            _ => return Json(json!({ "error": "Fly.io PAT not configured (add via Settings > Service Tokens or set FLY_API_TOKEN env var)" })),
        },
    };
...
pub async fn activity_logs(
    ...
    let total: i64 = sqlx::query_scalar("SELECT COUNT(*)::bigint FROM ch_sessions")
        .fetch_one(&state.db)
        .await
        .unwrap_or(0);
    ...
    let msg_counts: std::collections::HashMap<uuid::Uuid, i64> = if !session_ids.is_empty() {
        sqlx::query_as::<_, (uuid::Uuid, i64)>(
            "SELECT session_id, COUNT(*)::bigint FROM ch_messages WHERE session_id = ANY($1) GROUP BY session_id",
        )
        .bind(&session_ids)
        .fetch_all(&state.db)
        .await
