...
// ClaudeHydra v4 - Validates the generated OpenAPI schema

use utoipa::OpenApi;

#[test]
fn openapi_schema_is_valid_json() {
    let schema = serde_json::to_string_pretty(&claudehydra_backend::ApiDoc::openapi())
...
fn openapi_schema_contains_required_fields() {
    ...
    assert!(schema.contains("ClaudeHydra"), "Schema should contain project name");
}
...
fn openapi_schema_documents_key_endpoints() {
    let schema = serde_json::to_string_pretty(&claudehydra_backend::ApiDoc::openapi())
        .expect("OpenAPI schema should serialize to JSON");
    assert!(schema.contains("/api/claude/chat"), "Schema should document /api/claude/chat");
...
fn openapi_schema_parses_to_valid_structure() {
    let doc = claudehydra_backend::ApiDoc::openapi();
